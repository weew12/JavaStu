...
public class DataRecord  
{
    ...
    public static String getRecords() throws Exception{
        ...
        // 加上游戏提示
        result = "(按照历史耗时，翻牌数排名，耗时、翻牌数少的排名在前！)\n" + "1-简单模式 2-普通模式 3-困难模式(不计时为0)\n" + result;
        return result;
    }
    ...
    public static void saveData(long timere, int time, int counter, int type) throws Exception {
        ...
        try {
            ...
            Comparator<Map<String, Long>> comparator = (o1, o2) -> {
                ...
                int flipCards1 = o1.get("flipCards").intValue();int flipCards2 = o2.get("flipCards").intValue();
                if (costTime2 == costTime1) {
                    return flipCards1 - flipCards2;
                } else {
            ...
            item3.put("flipCards", (long)counter);
        ...
        try {
            ...
            while (outer.hasNext()) {
                ...
                String output = "|Record:" + String.format("%-15d", outrecord.get("Record"))
                        ...
                        + "|flipCards:" + String.format("%-3d", outrecord.get("flipCards"))
